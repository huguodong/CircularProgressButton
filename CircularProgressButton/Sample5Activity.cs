...
{
    [Activity(Label = "Sample5Activity")]
    public class Sample5Activity : Activity,CirButton.IOnClickListener
    {
        CirButton circularButton1;
        CirButton circularButton2;
        public static void startThisActivity(Activity activity)
        {
            activity.StartActivity(new Intent(activity, typeof(Sample5Activity)));

        }
        protected override void OnCreate(Bundle bundle)
        {
            base.OnCreate(bundle);
            SetContentView(Resource.Layout.ac_sample_5);
            ActionBar actionBar = ActionBar;
            if (actionBar != null)
            {
                actionBar.SetTitle(Resource.String.CustomSelectorSample);
            }
